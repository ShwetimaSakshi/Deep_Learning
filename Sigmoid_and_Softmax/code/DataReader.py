...
def prepare_X(raw_X):
	...
	### YOUR CODE HERE
    # flipped_raw_image= np.flip(raw_image, 2)
    flipped_raw_image= np.fliplr(raw_image)
    feature1 =  - ( np.sum(np.abs(raw_image - flipped_raw_image), (1,2)))/256
	### END YOUR CODE

	# Feature 2: Measure of Intensity
	### YOUR CODE HERE
    feature2 = np.sum(raw_image , (1,2))/256
	### END YOUR CODE

	# Feature 3: Bias Term. Always 1.
	### YOUR CODE HERE
    feature3 = np.array([1]*(raw_image.shape[0]))
	### END YOUR CODE

	# Stack features together in the following order.
	# [Feature 3, Feature 1, Feature 2]
	### YOUR CODE HERE
    all_features = np.stack((feature3, feature1, feature2), axis= 1)
	### END YOUR CODE
    return all_features
