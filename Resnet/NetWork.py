...
class ResNet(nn.Module):
    def __init__(self,
        ...
        ### YOUR CODE HERE
        # define conv1
        self.start_layer = nn.Conv2d(3, first_num_filters, kernel_size=3, stride=1, padding=1, bias=False)
...
class batch_norm_relu_layer(nn.Module):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.997) -> None:
        ...
        ### YOUR CODE HERE
        self.batch_norm_layer = nn.BatchNorm2d(num_features, eps=eps, momentum=momentum)
        self.relu_layer = nn.ReLU()

        ### YOUR CODE HERE
    def forward(self, inputs: Tensor) -> Tensor:
        ### YOUR CODE HERE
        output_forward_pass = self.batch_norm_layer(inputs)
        output_forward_pass = self.relu_layer(output_forward_pass)
...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        ### YOUR CODE HERE
        self.batch_norm_relu_layer = batch_norm_relu_layer(num_features=filters)
        # print('here shwetima', strides)
        self.conv1_layer = nn.Conv2d(filters, filters, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer1_value = nn.Sequential(self.conv1_layer, self.batch_norm_relu_layer)
        self.conv2_layer = nn.Conv2d(filters, filters, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer2_value = nn.Sequential(self.conv2_layer, nn.BatchNorm2d(filters,eps=1e-5,momentum=0.997))
        self.relu_layer = nn.ReLU()
        ### YOUR CODE HERE
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        residual_block = None
        if self.projection_shortcut is not None:
            residual_block = self.projection_shortcut(inputs)
        else:
            residual_block = inputs      
        residual_block_output = self.layer1_value(residual_block)
        residual_block_output = self.layer2_value(residual_block_output)
        residual_block_output = residual_block_output + residual_block
        residual_block_output = self.relu_layer(residual_block_output)
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.batch_norm_relu_layer1 = batch_norm_relu_layer(filters)
        self.conv1_layer = nn.Conv2d(filters, new_filters, kernel_size=1, stride=1, padding=0, bias=False)
        self.layer1_value = nn.Sequential(self.batch_norm_relu_layer1, self.conv1_layer)
        self.batch_norm_relu_layer2 = batch_norm_relu_layer(new_filters)
        self.conv2_layer = nn.Conv2d(new_filters,new_filters, kernel_size=3, stride=1, padding=1, bias=False)
        self.layer2_value = nn.Sequential(self.batch_norm_relu_layer2, self.conv2_layer)
        self.batch_norm_relu_layer3 = batch_norm_relu_layer(new_filters)
        self.conv3_layer = nn.Conv2d(new_filters, filters, kernel_size=1, stride=1, padding=0, bias=False)
        self.layer3_value = nn.Sequential(self.batch_norm_relu_layer3, self.conv3_layer)
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        residual_block_output = self.layer1_value(residual_block)
        residual_block_output = self.layer2_value(residual_block_output)
        residual_block_output = self.layer3_value(residual_block_output)
        residual_block_output = residual_block_output + residual_block
...
class stack_layer(nn.Module):
    ...
    def __init__(self, filters, block_fn, strides, resnet_size, first_num_filters) -> None:
        ...
        if filters != first_num_filters:
            filter_value = filters_out//2 
        else:
            filter_value = filters
        projection_shortcut = nn.Conv2d(filter_value,filters_out, kernel_size=1,stride= strides, bias=False)
        self.blocks_combining_layers = nn.ModuleList()
        for i in range(0,resnet_size):
            if(i != 0):
                projection_shortcut_value = None
                strides_value = 1
            else:
                projection_shortcut_value = projection_shortcut
                strides_value = strides
            self.blocks_combining_layers.append(block_fn(filters_out, projection_shortcut_value, strides_value, first_num_filters))
        ### END CODE HERE
    
    def forward(self, inputs: Tensor) -> Tensor:
        ### END CODE HERE
        stack_layer_output = inputs
        for block in self.blocks_combining_layers:
            stack_layer_output = block(stack_layer_output)
        return stack_layer_output
        ### END CODE HERE
...
class output_layer(nn.Module):
    ...
    def __init__(self, filters, resnet_version, num_classes) -> None:
        ...
        ### END CODE HERE
        # used this as the output from the output layer shape size had discrepancy
        if resnet_version == 1:
            filters = filters // 4
        self.num_classes = num_classes
        self.average_value = nn.AdaptiveAvgPool2d(1)
        self.classes_value = nn.Linear(filters, num_classes)
        ### END CODE HERE
    
    def forward(self, inputs: Tensor) -> Tensor:
        ### END CODE HERE
        output_layer_output = inputs
        if (self.resnet_version == 2):
            output_layer_output = self.bn_relu(output_layer_output)
        output_layer_output = self.average_value(output_layer_output)
        # Flatten the output for fully connected layer
        output_layer_output = output_layer_output.view(output_layer_output.size(0), -1)  
        output_layer_output = self.classes_value(output_layer_output)
